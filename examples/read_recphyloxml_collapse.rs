...
fn main() {
    ...
    read_recphyloxml_multi(input_file,&mut sp_tree, &mut gene_trees, &mut global_roots);
    recphyloxml_processing(&mut sp_tree, &mut gene_trees, &mut options, &config, true,
         &transfers, "read_recphyloxml_not_collapsed_2.svg".to_string());
    println!("Please open output file 'read_recphyloxml_not_collapsed_2.svg' with your browser");
    ...
    read_recphyloxml_multi(input_file2, &mut sp_tree, &mut gene_trees, &mut global_roots);
    recphyloxml_processing(&mut sp_tree, &mut gene_trees, &mut options, &config, true,
         &transfers, "read_recphyloxml_collapsed_2.svg".to_string());
    println!("Please open output file 'read_recphyloxml_collapsed_2.svg' with your browser");
    ...
    read_recphyloxml_multi(input_file,&mut sp_tree, &mut gene_trees, &mut global_roots);
    recphyloxml_processing(&mut sp_tree, &mut gene_trees, &mut options, &config, true,
         &transfers, "read_recphyloxml_not_collapsed_3.svg".to_string());
    println!("Please open output file 'read_recphyloxml_not_collapsed_3.svg' with your browser");

    // Version collapsed

    options.collapsed_nodes.push("0".to_string());
    let mut sp_tree: ArenaTree<String> = ArenaTree::default();
    let mut gene_trees:std::vec::Vec<ArenaTree<String>> = Vec::new();
    let mut global_roots: std::vec::Vec<usize> = Vec::new();
    read_recphyloxml_multi(input_file2, &mut sp_tree, &mut gene_trees, &mut global_roots);
    recphyloxml_processing(&mut sp_tree, &mut gene_trees, &mut options, &config, true,
         &transfers, "read_recphyloxml_collapsed_3.svg".to_string());
    println!("Please open output file 'read_recphyloxml_collapsed_3.svg' with your browser");
