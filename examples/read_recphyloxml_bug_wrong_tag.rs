...
fn main() {
    ...
    read_recphyloxml_multi("recphylo_examples/example_wrongtag.recphylo".to_string(),&mut sp_tree, &mut gene_trees, &mut global_roots);
    recphyloxml_processing(&mut sp_tree, &mut gene_trees, &mut options, &config, true,
         &transfers, "example_wrongtag.svg".to_string());
