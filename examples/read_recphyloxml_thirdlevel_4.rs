...
fn main() {
    ...
    let transfers = vec![];
    let infile_gs = "examples/test3/recgs_dtl.recphyloxml".to_string();
    let infile_sh = "examples/test3/rechp_dtl.recphyloxml".to_string();
    let _infile_gs = infile_gs.clone();
    ...
    // // genes trees
    // // ---------------------------------------------------------
    // read_recphyloxml_multi(_infile_gs,&mut _global_pipe_parasite,&mut path_genes,&mut _global_roots);
