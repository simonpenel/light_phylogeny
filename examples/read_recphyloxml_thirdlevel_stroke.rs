...
fn main() {
    ...
    phyloxml_processing(
        ...
        &mut options,
        &config,
        "para_simple_stroke.svg".to_string(),
    );
    reset_pos(&mut tree_host_pipe);
    phyloxml_processing(
        &mut tree_host_pipe,
        &mut options,
        &config,
        "host_simple_stroke.svg".to_string(),
    );
    let mut i = 0;
    while i < nb_parasite_pipe {
        reset_pos(&mut path_para_trees[i]);
        phyloxml_processing(
            ...
            &mut options,
            &config,
            ("gene_simple_".to_owned() + &i.to_string() + "_stroke.svg").to_string(),
        );
        i = i + 1;
    }
    ...
    while i < nb_parasite_pipe {
        println!(
            " - para_simple_stroke.svg ...... 2 levels: gene_simple_{}.svg",
            &i
        );
        i = i + 1;
    }
    ...
    println!(
        " - gene_para_stroke.svg ........ 2 levels: pipe parasite tree(s) with gene tree(s) inside"
    );
    println!(
        " - para_host_stroke.svg ........ 2 levels: pipe host tree with parasite tree(s) inside"
    );
