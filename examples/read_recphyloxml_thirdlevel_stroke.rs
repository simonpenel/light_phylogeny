...
fn main() {
    ...
    options.squaresize = 30.0;    
    options.gthickness = 9;
    ...
    let  outfile_gene_para = String::from("gene_para_stroke.svg");
    let  outfile_para_host = String::from("para_host_stroke.svg");
    let  outfile_mapped_1 = String::from("mapped_1_stroke.svg");
    let  outfile_mapped_2 = String::from("mapped_2_stroke.svg");
...
reset_pos(&mut global_pipe_parasite);
phyloxml_processing(&mut global_pipe_parasite, &mut options, &config,"para_simple_stroke.svg".to_string());
reset_pos(&mut tree_host_pipe);
phyloxml_processing(&mut tree_host_pipe, &mut options, &config,"host_simple_stroke.svg".to_string());
let mut i = 0;
while i < nb_parasite_pipe {
reset_pos(&mut path_para_trees[i]);
phyloxml_processing(&mut path_para_trees[i], &mut options, &config,("gene_simple_".to_owned()+&i.to_string()+"_stroke.svg").to_string());
i = i + 1;
}
...
true, &vec![],"mapped_3_stroke.svg".to_string());
// println!("DEBUG  FINAL HOST = {:?}",tree_host_pipe);
// lmlmlmlm

println!("Output files:");

println!(" - host_simple_stroke.svg ...... 1 level: host tree");
let mut i = 0;
while i < nb_parasite_pipe {
println!(" - para_simple_stroke.svg ...... 2 levels: gene_simple_{}.svg",&i);
i = i + 1;
}
println!(" - para_simple_stroke.svg ...... 2 levels: parasite tree(s)");
println!(" - gene_para_stroke.svg ........ 2 levels: pipe parasite tree(s) with gene tree(s) inside");
println!(" - para_host_stroke.svg ........ 2 levels: pipe host tree with parasite tree(s) inside");
println!(" - mapped_1_stroke.svg ........  3 levels: reconciled pipe parasite tree(s) with gene tree(s)");
println!(" - mapped_2_stroke.svg ........  3 levels: parasite-host reconciliation plus gene transfers");
println!(" - mapped_3_stroke.svg ........  3 levels: pipe host tree with gene tree(s) inside");
