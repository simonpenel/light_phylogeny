    ...
    T: PartialEq + std::default::Default + std::fmt::Debug,
{
    ...
    pub fn get_largest_y_nofl(&mut self) -> f32 {
        ...
        for node in &self.arena {
            if node.y + node.height / 2.0 > max
                && node.location != "FREE_LIVING".to_string()
    ...
    pub fn shift_x_subtree(&mut self, idx: usize, shift: f32) {
        ...
        x = x + shift;
        debug!(
            "[shift_x_subtree] shifting node {} ({}) of {}",
            &self.arena[idx].name, &self.arena[idx].x, shift
        );
        let _ = &self.arena[idx].set_x_noref(x);
        debug!(
            "[shift_x_subtree] new value     {} ({})",
            &self.arena[idx].name, &self.arena[idx].x
        );
...
pub fn xml2tree(
    ...
    for child in children {
        ...
        if child.has_tag_name("events") {
            info!(
                "[xml2tree]  phyloXML event detected at {:?}",
                tree.arena[parent]
            );
        ...
        if child.has_tag_name("eventsRec") {
            info!(
                "[xml2tree] recPhyloXML event detected at {:?}",
                tree.arena[parent]
            );
            ...
            for evenement in child.children() {
                ...
                if evenement.has_tag_name("speciationLoss") {
                    ...
                    info!(
                        "[xml2tree] Find obsolete tag speciationLoss (# {})",
                        sploss_num
                    );
                    info!(
                        "[xml2tree] Index of current father (current_sploss_name)  {}",
                        current_sploss_name
                    );
                    ...
                    *numero += 1;
                    ...
                    assert_eq!(
                        evenement.attributes().nth(0).unwrap().name(),
                        "speciesLocation"
                    );
                ...
                if evenement.has_tag_name("leaf") {
                    ...
                    if nb_att == 1 {
                        ...
                        if evenement.attributes().nth(0).unwrap().name() == "geneName" {
                            assert_eq!(
                                evenement.attributes().nth(1).unwrap().name(),
                                "speciesLocation"
                            );
                ...
                if evenement.has_tag_name("bifurcationOut") {
                    ...
                    match grandparent {
                        Some(p) => {
                            ...
                            info!(
                                "[xml2tree] set location according to its father = {}",
                                location
                            );
                            tree.arena[parent].location = location.to_string();
                        }
                        None => panic!(
                            "BifurcationOut node as no parent : {:?}",
                            tree.arena[parent]
                        ),
                    };
                    tree.arena[parent].is_a_transfert = true;
                ...
                if flag_correct_tag_found == false {
                    eprintln!(
                        "\nERROR: The event [{:?}] is not a recPhyloXML standard event.",
                        evenement.tag_name()
                    );
...
pub fn check_for_obsolete(gene_tree: &mut ArenaTree<String>, species_tree: &mut ArenaTree<String>) {
    ...
    for index in &mut gene_tree.arena {
        if index.e == Event::ObsoleteSpeciationLoss {
            info!(
                "[check_for_obsolete] Find ObsoleteSpeciationLoss (OSL): {:?}",
                index
            );
            osls.push(index.idx);
        }
    ...
    if osls.len() > 0 {
        println!(
            "Warning! There was {} obsolete SpeciationLoss tag in the recPhyloXML",
            osls.len()
        );
    }
    for osl in osls {
        ...
        let right = children[1];
        info!(
            "[check_for_obsolete] Left child = {} {:?}",
            gene_tree.arena[left].name, gene_tree.arena[left].e
        );
        info!(
            "[check_for_obsolete] Right child = {} {:?}",
        ...
        info!(
            "[check_for_obsolete] Loss node is {:?}",
            gene_tree.arena[loss]
        );
        info!(
            "[check_for_obsolete] Species of the other child is {:?}",
            species_not_loss
        );
        // Espece de l'OSL
        ...
        let species_node = match species_tree.get_index(species.to_string()) {
            Ok(index) => index,
            Err(_err) => panic!(
                "[check_for_obsolete] Unable to find node {:?}",
                species.to_string()
            ),
        };
        ...
        info!(
            "[check_for_obsolete] Species under the OSL in species tree = {}, {}",
        ...
        info!(
            "[check_for_obsolete] Thus species of loss node is {}",
            loss_species
        );
        ...
        for index in &mut gene_trees[idx_rcgen].arena {
            ...
            for spindex in &mut sp_tree.arena {
                if index.location == spindex.name {
                    ...
                    index.y = y;
                    info!(
                        "[map_gene_trees] [{}] Gene node {:?} mapped to  species node {:?}",
...
pub fn map_species_trees(
    ...
    let nb_gntree = gene_trees.len(); // Nombre d'arbres de gene
    info!(
        "[map_species_trees] {} gene trees to be processed",
        nb_gntree
    );
    if nb_gntree > 0 {
        let mut idx_rcgen = 0; // Boucle sur les arbres de genes
            ...
            for index in &mut gene_trees[idx_rcgen].arena {
                ...
                for spindex in &mut sp_tree.arena {
                    if index.location == spindex.name {
                        ...
                        info!(
                            "[map_species_trees] Gene node {:?} mapped to  species node {:?}",
                            index, spindex
                        );
                    }
                }
                if !mapped {
                    ...
                    eprintln!(
                        "\nThe species '{}' was not found in species tree",
                        index.location
                    );
...
pub fn bilan_mappings_reti(
    ...
    info!(
        "[bilan_mappings_reti] Species Node {}",
        sp_tree.arena[index].name
    );
            ...
            Event::Duplication => {
                ...
                if options.mid_dist {
                    ...
                    let midistance = match species_parent {
                        ...
                        Some(p) => {
                            (gene_trees[*index_node].arena[*node].y
                                - gene_trees[*index_node].arena[p].y)
                                / 1.5
                        }
                    };
...
pub fn process_fl(
    ...
    for (index_node, node) in feuilles {
        ...
        let mut parent = gene_trees[*index_node].arena[*node]
            .parent
            .expect("[process_fl] ERROR Unexpected root (1)");
        while gene_trees[*index_node].arena[*node].location
            == gene_trees[*index_node].arena[parent].location
        {
            _parent = parent;
        ...
        info!(
            "[process_fl] Ancestor of the gene {} in this species node is {:?}",
        ...
        let mut parent_xmod =
            sp_tree.arena[index].x - sp_tree.arena[index].width + shift * PIPEBLOCK;
...
pub fn copie_fl(fl_tree: &mut ArenaTree<String>, gn_tree: &mut ArenaTree<String>, index: usize) {
    ...
    if gn_tree.arena[index].location == "FREE_LIVING"
        && gn_tree.arena[index].e != Event::BranchingOut
    {
...
pub fn center_gene_nodes(
    ...
    let up_sp =
        sp_tree.arena[index].y + sp_tree.arena[index].ymod - sp_tree.arena[index].height / 2.0;
    let down_sp =
        sp_tree.arena[index].y + sp_tree.arena[index].ymod + sp_tree.arena[index].height / 2.0;
    ...
    for spindex in &sp_tree.arena {
        ...
        if spindex.children.len() == 0 {
            for (index_node, node) in &sp_tree.arena[spindex.idx].nodes {
                ...
                if gene_trees[*index_node].arena[*node].location != "FREE_LIVING".to_string()
                    && gene_trees[*index_node].arena[*node].e == Event::Leaf
                {
...
pub fn find_sptrees(doc: &mut roxmltree::Document) -> Result<Vec<roxmltree::NodeId>, usize> {
    ...
    info!(
        "[find_sptrees] Number of species trees in xml = {}",
        gene_nodes.len()
    );
...
pub fn find_rgtrees(doc: &mut roxmltree::Document) -> Result<Vec<roxmltree::NodeId>, usize> {
    ...
    info!(
        "[find_rgtrees] Number of gene trees in xml = {}",
        gene_nodes.len()
    );
    match gene_nodes.len() > 0 {
        true => return Ok(gene_nodes),
...
pub fn fusion_mod_xy(tree: &mut ArenaTree<String>, index_1: usize, index_2: usize) {
    if ((!tree.is_leaf(index_1)) && (tree.is_leaf(index_2)))
        || ((tree.is_leaf(index_1)) && (!tree.is_leaf(index_2)))
    {
        eprintln!(
            "[fusion_mod_xy] ERROR Unable to merge {} and {}.",
            tree.arena[index_1].name, tree.arena[index_2].name
        );
    ...
    info!(
        "[fusion_mod_xy] Before : x of {} = {}.",
        &tree.arena[index_1].name, tree.arena[index_1].x
    );
    info!(
        "[fusion_mod_xy] Before : x of {} = {}.",
        &tree.arena[index_2].name, tree.arena[index_2].x
    );
    tree.arena[index_1].x = dist;
    tree.arena[index_2].x = dist;
    info!(
        "[fusion_mod_xy] After : x of {} = {}.",
        &tree.arena[index_1].name, tree.arena[index_1].x
    );
    info!(
        "[fusion_mod_xy] After : x of {} = {}.",
    ...
    info!(
        "[fusion_mod_xy] Before : width of {} = {}.",
        &tree.arena[index_1].name, tree.arena[index_1].width
    );
    info!(
        "[fusion_mod_xy] Before : width of {} = {}.",
        &tree.arena[index_2].name, tree.arena[index_2].width
    );
    ...
    if (!tree.is_leaf(index_1)) && (!tree.is_leaf(index_2)) {
        ...
        info!(
            "[fusion_mod_xy] Before : y of {} = {}.",
            &tree.arena[index_1].name, tree.arena[index_1].y
        );
        info!(
            "[fusion_mod_xy] Before : y of {} = {}.",
            &tree.arena[index_2].name, tree.arena[index_2].y
        );
...
pub fn shift_mod_xy(tree: &mut ArenaTree<String>, index: usize, xmod: &mut f32, ymod: &mut f32) {
    debug!(
        "[shift_mod_xy] shifting {:?} xmod={} ymod={}",
        tree.arena[index], xmod, ymod
    );
...
pub fn explore_postorder(tree: &mut ArenaTree<String>, index: usize) {
    ...
    if children.len() > 0 {
        ...
        println!(
            "POST-ORDER TRAVERSAL : INTERNAL NODE  {:?} / DEPTH = {}",
            tree.arena[index],
            tree.depth(index)
        );
    } else {
        println!(
            "POST-ORDER TRAVERSAL : LEAF           {:?} / DEPTH = {}",
            tree.arena[index],
            tree.depth(index)
        );
    }
...
pub fn check_vertical_contour_postorder(
    ...
    if children.len() > 0 {
        ...
        let right = children[1];
        debug!(
            "[check_vertical_contour_postorder] Father = {} (ymod = {} ) , Left = {}, Right = {}",
            tree.arena[index].name,
            tree.arena[index].ymod,
            tree.arena[left].name,
        ...
        check_vertical_contour_postorder(
            tree,
            left,
            tree.arena[left].ymod + 0.0 * ymod,
            compression,
        );
        check_vertical_contour_postorder(
            tree,
            right,
            tree.arena[right].ymod + 0.0 * ymod,
            compression,
        );
    }
}
...
pub fn push_down(
        ...
        || (node_right_up_pos <= node_parent_down_pos + compression * PIPEBLOCK)
    {
        ...
        debug!(
            "[push_down] SHIFTING Y {} + 1xPIPEBLOCK = {}",
            shift_down,
            shift_down + 1.0 * PIPEBLOCK
        );
        debug!(
            "Initial left : y = {}, ymod = {}",
            tree.arena[left].y, tree.arena[left].ymod
        );
...
pub fn check_contour_postorder_tidy_tree(
    ...
    info!(
        "[check_contour_postorder_tidy_tree] node {}",
        tree.arena[index].name
    );
    let children = &mut tree.arena[index].children;
    if children.len() > 0 {
...
pub fn get_contour_left(
    ...
    if contour_left.len() <= local_depth {
        ...
        debug!(
            "[get_contour_left] increment contour is now {:?}",
            contour_left
        );
    }
    if tree.arena[index].xmod < 0.0 {
    ...
    if children.len() > 0 {
        ...
        get_contour_left(
            ...
            depth,
            contour_left,
            tree.arena[index].xmod + parent_xmod,
        );
    }
}
...
pub fn get_contour_right(
    ...
    if contour_right.len() <= local_depth {
        ...
        debug!(
            "[get_contour_right] increment contour is now {:?}",
            contour_right
        );
    }
    if tree.arena[index].xmod < 0.0 {
        ...
        y = y + match options.tidy_leaves_check {
            ...
            true => {
                match tree.arena[index].nbg > 0 {
                    ...
                    false => {
                        tree.arena[index].name.len() as f32
...
pub fn push_right(tree: &mut ArenaTree<String>, left_tree: usize, right_tree: usize) -> f32 {
    debug!(
        "[push_right] compare right contour of {} and left contour of {}",
        left_tree, right_tree
    );
    let mut right_co_of_left_tr = vec![
        tree.arena[left_tree].x
            + tree.arena[left_tree].xmod
            + tree.arena[left_tree].nbg as f32 * PIPEBLOCK,
    ];
    ...
    debug!(
        "[push_right] right contour of {} = {:?}",
        left_tree, right_co_of_left_tr
    );
    let mut left_co_of_right_tr = vec![
        tree.arena[right_tree].x + tree.arena[right_tree].xmod
            - tree.arena[right_tree].nbg as f32 * PIPEBLOCK,
    ];
    ...
    debug!(
        "[push_right] left contour of {} = {:?}",
        right_tree, left_co_of_right_tr
    );
    ...
    if left_len > right_len {
        ...
        debug!(
            "[push_right] complete right contour with last value {}",
            last_val
        );
    }
    if left_len < right_len {
        ...
        debug!(
            "[push_right] complete left contour with last value {}",
            last_val
        );
    }
    debug!(
        "[push_right] comparing  right cont. of left tree: {:?}",
        right_co_of_left_tr
    );
    debug!(
        "[push_right] with left cont. of right tree:       {:?} ",
        left_co_of_right_tr
    );
    let iter = left_co_of_right_tr
        .iter()
        .zip(right_co_of_left_tr)
        .map(|(x, y)| (x - y));
...
pub fn dmin_tidy(
    ...
    debug!(
        "[dmin_tidy] Compare x value of  right {} and  left  {}",
    ...
    debug!(
        "[dmin_tidy] Compare y value of  right {} {} and  left  {} {}",
        cont_left[*index_left].2,
        cont_left[*index_left].1,
        cont_right[*index_right].2,
...
pub fn push_right_tidy_tree(
    ...
    debug!(
        "[push_right_tidy_tree] Compare right contour of {} {} and left contour of {} {}",
        tree.arena[left_tree].val,
        tree.arena[left_tree].name,
        tree.arena[right_tree].val,
    ...
    debug!(
        "[push_right_tidy_tree] Contour right = {:?}",
        right_co_of_left_tr
    );
    ...
    debug!(
        "[push_right_tidy_tree] Contour left = {:?}",
        left_co_of_right_tr
    );
    ...
    debug!(
        "[push_right_tidy_tree] DMIN = {:?} [max = {}]",
        dmin, PIPEBLOCK
    );
    if dmin > BLOCK * 0.50 {
        debug!(
            "[push_right_tidy_tree] SHIFTING SUBTREE  {:?} {:?}",
...
pub fn set_middle_postorder(tree: &mut ArenaTree<String>, index: usize) {
    ...
    if children.len() > 0 {
        ...
        debug!(
            "[set_middle_postorder] x father set from {} to {}",
            x, x_middle
        );
...
pub fn lca(tree: &mut ArenaTree<String>, index1: usize, index2: usize) -> usize {
    ...
    if d1 == d2 {
        // lca(tree,p1,p2);
        info!(
            "[lca] Comparison of {:?} and {:?}",
            tree.arena[index1], tree.arena[index2]
        );
        match p1 == p2 {
            true => {
                info!("[lca] LCA is {}", p1);
...
pub fn get_x_span(tree: &ArenaTree<String>, index: usize) -> (f32, f32) {
    ...
    for index in descendants {
        if tree.arena[index].x + tree.arena[index].width / 2.0 > max_x {
            max_x = tree.arena[index].x + tree.arena[index].width / 2.0
        }
        if tree.arena[index].x - tree.arena[index].width / 2.0 < min_x {
            min_x = tree.arena[index].x - tree.arena[index].width / 2.0
        }
    }
    (min_x, max_x)
}
...
pub fn summary(tree: &mut ArenaTree<String>) {
    for index in &tree.arena {
        ...
        match children.len() > 0 {
            ...
            false => {
                print!(
                    "Node {} ({}) has no children and its parent is {}",
                    index.idx, index.name, parent
                )
            }
        }
        println!(" [{:?}]", index.e);
...
pub fn move_child(tree: &mut ArenaTree<String>, child: usize, new_parent: usize) {
    ...
    info!(
        "[move_child] new parent node : {:?}",
        tree.arena[new_parent]
    );
}
