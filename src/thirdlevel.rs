    ...
    for (start, end) in transfers {
        ...
        while i < nb_par {
            ...
            match map_start {
                Ok(_) => {
                    info!(
                        "[map_transfer_mul] Find transfert start in parasite tree {}",
                        i
                    );
                    break;
                }
        ...
        while i < nb_par {
            ...
            match map_end {
                Ok(_) => {
                    info!(
                        "[map_transfer_mul] Find transfert end in parasite tree {}",
                        i
                    );
                    break;
                }
    ...
    for index in &mut para_as_species.arena {
        ...
        match i {
            ...
            Ok(i) => {
                ...
                info!(
                    "[map_parasite_g2s] Mapping of {} OK, event is {:?} (transfert: {})",
                    name, e, &para_as_gene.arena[i].is_a_transfert
                );
    ...
    for index in &para_as_gene.arena {
        ...
        match i {
            ...
            Err(_err) => {
                ...
                info!(
                    "[map_parasite_s2g] Unable to map {} {:?}, new node needed",
                    name, &index.e
                );
                ...
                let p = match p {
                    ...
                    None => {
                        ...
                        eprintln!("");
                        eprintln!(
                            "The following node in the  reconciled 'lower' tree was not found \
                        in the 'upper' tree:\n{:?} ",
                            index
                        );
                ...
                info!(
                    "[map_parasite_s2g] parent of the node in the  parasite 'lower' tree \
                ...
                info!(
                    "[map_parasite_s2g] Parents: {:?} <=> {:?}",
                    para_as_species.arena[j], para_as_gene.arena[*p]
                );
                info!(
                    "[map_parasite_s2g]  Event of the current parasite 'lower' tree node is {:?}",
                    index.e
                );
                match index.e {
                    ...
                    Event::Loss => {
                        ...
                        info!(
                            "[map_parasite_s2g]  LOSS ADDED {:?}",
                            para_as_species.arena[new_loss]
                        );
                        info!(
                            "[map_parasite_s2g]  => parent is now : {:?}",
                            para_as_species.arena[j]
                        );
                    }
                    Event::Leaf => {
                        ...
                        info!(
                            "[map_parasite_s2g] parent species node is now #{} {:?}",
                            j, para_as_species.arena[j]
                        );
                        info!(
                            "[map_parasite_s2g] added species leave is now #{} {:?}",
                            new_leaf, para_as_species.arena[new_leaf]
                        );
                        info!(
                            "[map_parasite_s2g] the reconciled species node is still {:?}",
                            index
                        );
                        ...
                        info!(
                            "[map_parasite_s2g] Loop over associated nodes {:?}",
                            new_gnodes
                        );
                        ...
                        for (ng, nn) in new_gnodes {
                            ...
                            info!("\n[map_parasite_s2g]");
                            info!(
                                "[map_parasite_s2g] Modify gene tree number {}, node {}",
                                *ng, nn
                            );
                            info!(
                                "[map_parasite_s2g] Before : {:?}",
                                &gene_trees[*ng].arena[*nn]
                            );
                            match gene_trees[*ng].arena[*nn].parent {
                                Some(p) => info!(
                                    "[map_parasite_s2g] Parent = {:?}:",
                                    &gene_trees[*ng].arena[p]
                                ),
                                None => {}
                            };
                            ...
                            if sons.len() > 0 {
                                info!(
                                    "[map_parasite_s2g] Left =  {:?} ",
                                    &gene_trees[*ng].arena[sons[0]]
                                );
                                info!(
                                    "[map_parasite_s2g] Right = {:?} ",
                                    &gene_trees[*ng].arena[sons[1]]
                                );
                            }
                            // ng is the tree number, nn the index of node
                            ...
                            info!(
                                "[map_parasite_s2g] Previous {:?}:",
                                &gene_trees[*ng].arena[*nn]
                            );
                            gene_trees[*ng].arena[*nn].location =
                                para_as_species.arena[new_leaf].name.to_string();
                            ...
                            if nn > &0 {
                                ...
                                if gene_trees[*ng].arena[parent].e == Event::BranchingOut {
                                    continue;
                                };
                                if gene_trees[*ng].arena[parent].e == Event::Duplication {
                                    continue;
                                };
                                ...
                                let new_svgnode = gene_trees[*ng]
                                    .new_node("virtualsvg_".to_string() + &virt_svg.to_string());
                                gene_trees[*ng].arena[new_svgnode].name =
                                    "virtualsvg_".to_string() + &virt_svg.to_string();
                                virt_svg = virt_svg + 1;
                                // A verifier
                                gene_trees[*ng].arena[new_svgnode].location = index.name.clone();
                                gene_trees[*ng].arena[new_svgnode].virtualsvg = true;
                                gene_trees[*ng].arena[new_svgnode].is_a_transfert =
                                    gene_trees[*ng].arena[parent].is_a_transfert;
                                ...
                                // nouveau noeud 2
                                let new_svgnode_bis = gene_trees[*ng]
                                    .new_node("virtualsvg_".to_string() + &virt_svg.to_string());
                                gene_trees[*ng].arena[new_svgnode_bis].name =
                                    "virtualsvg_".to_string() + &virt_svg.to_string();
                                virt_svg = virt_svg + 1;
                                // A verifier
                                gene_trees[*ng].arena[new_svgnode_bis].location =
                                    index.name.clone();
                                ...
                                gene_trees[*ng].arena[new_svgnode]
                                    .children
                                    .push(new_svgnode_bis);
                                ...
                                info!(
                                    "[map_parasite_s2g] Previous node : {:?}",
                                    gene_trees[*ng].arena[*nn]
                                );
                                info!(
                                    "[map_parasite_s2g] Parent :    {:?}",
                                    gene_trees[*ng].arena[parent]
                                );
                                info!(
                                    "[map_parasite_s2g] Virtual 1 : {:?}",
                                    gene_trees[*ng].arena[new_svgnode]
                                );
                                info!(
                                    "[map_parasite_s2g] Virtual 2 : {:?}",
                                    gene_trees[*ng].arena[new_svgnode_bis]
                                );
                        ...
                        for (node, ng, (node1, node2)) in add_gnodes {
                            ...
                            info!(
                                "[map_parasite_s2g] Species  node  was {:?}",
                                para_as_species.arena[node]
                            );
                            ...
                            info!(
                                "[map_parasite_s2g] Species node is now {:?}",
                                para_as_species.arena[node]
                            );
                        }
                    }
                    _ => {
                        ...
                        for (ng, nn) in new_gnodes {
                            ...
                            gene_trees[*ng].arena[*nn].location =
                                para_as_species.arena[new_node].name.to_string();
                            info!("[map_parasite_s2g] New {:?}:", &gene_trees[*ng].arena[*nn]);
                            // In case the node is not a root (I expect index 0 is root, maybe this
                            // is not perfect) we need to add a gene node to map exactly with the
                            // pipe species, except if the node is transfer
        ...
        while j < nb_nodes {
            ...
            while k < nb_par {
                ...
                let espece_index = match espece_index {
                    Ok(n) => {
                        if find_spec {
                            panic!("[map_gene_host] Multiple species matches")
                        }
                        find_spec = true;
                    ...
                    Err(_e) => {
                        info!(
                            "species {} not found in parasite tree number {}",
                ...
                if find_spec {
                    ...
                    gene_trees[i].arena[j].location =
                        tree_para_pipes[k].arena[espece_index].location.clone();
                    ...
                    if tree_para_pipes[k].arena[espece_index].is_a_transfert {
                        ...
                        match parent {
                            None => {
                                eprintln!(
                                    "Note: Gene node {:?} has no parent",
                                    gene_trees[i].arena[j].name
                                );
                                eprintln!(
                                    "      but it is associated to host  node {:?}",
                                    gene_trees[i].arena[j].location
                                );
                                eprintln!(
                                    "      via species node {:?} which is a transfer.",
                                    tree_para_pipes[k].arena[espece_index].name
                                );
                                info!(
                                    "[map_gene_host] Node {:?} has no parent",
                                    gene_trees[i].arena[j]
                                );
                                info!("[map_gene_host] but it is associated to {:?} which is a transfer",gene_trees[i].arena[j].location);
                                info!(
                                    "[map_gene_host] ({:?})",
                                    tree_para_pipes[k].arena[espece_index]
                                );
                            }
            ...
            if !global_find_spec {
                panic!(
                    "[map_gene_host] Unable to map node {:?} loc={:?}",
                    gene_trees[i].arena[j].name.clone(),
                    gene_trees[i].arena[j].location.clone()
                )
            }
...
pub fn select_transfer(
    ...
    for (start, end) in transfers {
        let s = species_tree
            .get_index(start.to_string())
            .expect("[select_transfer] Unable fo find start");
        let e = species_tree
            .get_index(end.to_string())
            .expect("[select_transfer] Unable fo find start");
...
pub fn optimisation(transfer: &(String, String), species_tree: &mut ArenaTree<String>) -> usize {
    ...
    let s = species_tree
        .get_index(start.to_string())
        .expect("[optimisation] Unable fo find start");
    let e = species_tree
        .get_index(end.to_string())
        .expect("[optimisation] Unable fo find start");
    let ancestor = lca(species_tree, s, e);
    info!(
        "[optimisation] Ancestor of {}->{} is {}",
        start, end, &species_tree.arena[ancestor].name
    );
    ...
    info!(
        "[optimisation] Rigth son of ancestor is {}",
        &species_tree.arena[droite].name
    );
    ...
    if _right_left_start {
        ...
        info!(
            "[optimisation]  Start: Check if {} is not {} nor its parent {}",
            species_tree.arena[s].name,
            species_tree.arena[droite].name,
            species_tree.arena[ancestor].name
        );
        if (s != droite) && (s != ancestor) {
            info!(
                "[optimisation] Increment 'go left' at {}",
                species_tree.arena[s].name
            );
            species_tree.arena[s].go_left = species_tree.arena[s].go_left + 1;
            let mut parent = species_tree.arena[s].parent;
            info!(
                "[optimisation] Start: Add a go left from parent of {} to {}",
        ...
        info!(
            "[optimisation] End: Check if {} is not {} nor its parent {}",
            species_tree.arena[e].name,
            species_tree.arena[gauche].name,
            species_tree.arena[ancestor].name
        );
        if (e != gauche) && (e != ancestor) {
            info!(
                "[optimisation] Increment go right at {}",
                species_tree.arena[e].name
            );
            species_tree.arena[e].go_right = species_tree.arena[e].go_right + 1;
            let mut parent = species_tree.arena[e].parent;
            info!(
                "[optimisation] End: Add a go right from parent of {} to {}",
        ...
        if (s != gauche) && (s != ancestor) {
            info!(
                "[optimisation] Increment go rigth at {}",
                species_tree.arena[s].name
            );
            species_tree.arena[s].go_right = species_tree.arena[s].go_right + 1;
            let mut parent = species_tree.arena[s].parent;
            info!(
                "[optimisation] Start: Add a go right from parent of {} to {}",
        ...
        if (e != droite) && (e != ancestor) {
            ...
            let mut parent = species_tree.arena[e].parent;
            info!(
                "[optimisation] End: Add a go left from parent of {} to {}",
...
pub fn check_optimisation(
    ...
    info!(
        "[check_optimisation] Processing transfer {}->{} for node {}",
        start, end, &species_tree.arena[node].name
    );
    // est ce le start est a droite du end?
    let s = species_tree
        .get_index(start.to_string())
        .expect("[check_optimisation] Unable fo find start");
    let e = species_tree
        .get_index(end.to_string())
        .expect("[check_optimisation]] Unable fo find start");
    ...
    if children.len() > 0 {
        ...
        if switch {
            info!(
                "[check_optimisation] Try to switch node {} : ",
                species_tree.arena[node].name
            );
        ...
        if switch && (species_tree.arena[node].fixed == false) {
            info!(
                "[check_optimisation] Inversion at node {} {} ({}>{})",
                node, species_tree.arena[node].name, score_goleft_right, score_goleft_left
            );
        ...
        if (node != e) && (node != s) {
            info!(
                "[check_optimisation] Fixing node orientation {}",
                species_tree.arena[node].name
            );
...
pub fn classify_transfer(
    ...
    let s = species_tree
        .get_index(start.to_string())
        .expect("[classify_transfer] Unable fo find start");
    let e = species_tree
        .get_index(end.to_string())
        .expect("[classify_transfer] Unable fo find start");
    let ancestor = lca(species_tree, s, e);
...
pub fn reorder_transfers(
    ...
    info!(
        "[reorder_transfers_transfer] Transfers at {} =  {:?}",
        &species_tree.arena[node].name, tr
    );
    for t in tr {
