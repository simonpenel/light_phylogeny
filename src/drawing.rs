...
use crate::arena::{get_root_distance, get_x_span};
...
pub fn draw_tree(
    ...
    let height_svg =
        largest_y - smallest_y + longest_name * config.gene_police_size.parse::<f32>().unwrap();
    ...
    let mut document = match options.rotate {
        true => Document::new()
            ...
            .set(
                "style",
                "background-color:".to_owned() + &options.bckg_color,
            ),
        false => Document::new()
            .set("width", width_svg)
    ...
    // ------------------
    let style = Style::new(
        ".gene { font-size: ".to_owned()
            + &config.gene_police_size.to_string()
            + "px; fill:"
            + &gene_color.to_string()
            + "; } .support { font-size: "
            + &config.gene_police_size.to_string()
            + "px; fill: red; } .branch { font-size: "
            + &config.gene_police_size.to_string()
            + "px; fill: black; }",
    );
        ...
        for node_color in &options.node_colors {
            ...
            match node_colored {
                Ok(n) => {
                    ...
                    let added_style = " .node_".to_owned()
                        + &node_color_idx.to_string()
                        + " { font-size: "
                        + &config.gene_police_size.to_string()
                        + "px; fill:"
                        + &font_color.to_string()
    ...
    for index in &tree.arena {
        ...
        if index.collapsed {
            ...
            match event {
                Event::Leaf => g.append(get_half_circle(
                    index.x,
                    index.y,
                    options.squaresize,
                    gene_color.to_string(),
                ...
                Event::Duplication => g.append(get_carre(
                    index.x,
                    index.y,
                    options.squaresize,
                    gene_color.to_string(),
                ...
                Event::Hybridation => g.append(get_empty_circle(
                    index.x,
                    index.y,
        ...
        if index.visible {
            ...
            match tree.is_leaf(index.idx) {
                false => element.assign("x", index.x - 5.0),
                true => {
                    // Cas d'une feuille: on decale pour prendre en compte les timelines
                    element.assign(
                        "x",
                        index.x + options.time_lines.len() as f32 * width_timeline,
                    );
                }
            }
            ...
            if options.node_colors.len() > 0 {
                element.assign(
                    "class",
                    "node_".to_owned() + &index.color_node_idx.to_string(),
                );
            }
...
pub fn draw_sptree_gntrees(
    ...
    let mut recphylostyle: String = ".species { font: italic ".to_owned()
        + "; font-size: "
        + &config.species_police_size.to_string()
        + "px; fill: "
    ...
    loop {
        ...
        if options.node_colors.len() > 0 {
            ...
            for node_color in &options.node_colors {
                ...
                match node_colored {
                    ...
                    Err(_e) => {
                        println!(
                            "Warning Node {} not found in the gene tree number {}",
                            node_color, idx_rcgen
                        )
                    }
                }
                node_color_idx += 1;
        ...
        let added_style = " .gene_".to_owned()
            + &idx_rcgen.to_string()
            + " { font-size: "
            + &config.gene_police_size.to_string()
            + "px; fill:"
        ...
        for index in &gene_trees[idx_rcgen].arena {
                                ...
                                _ => {
                                    ...
                                    if index.virtualsvg {
                                        ...
                                        if n.virtualsvg {
                                            ...
                                            if nnn.e != Event::BranchingOut
                                                && nnn.e != Event::BifurcationOut
                                            {
            ...
            if index.visible {
                match event {
                    ...
                    Event::BifurcationOut => g.append(get_empty_circle(
                        index.x,
                        index.y,
                    ...
                    Event::Speciation => g.append(get_circle(
                        index.x,
                        index.y,
                        options.squaresize * 0.75,
                        gene_color.to_string(),
                    ...
                    Event::ObsoleteSpeciationLoss => g.append(get_circle(
                        index.x,
                        index.y,
                        options.squaresize * 0.75,
                        gene_color.to_string(),
            ...
            if index.visible {
                match event {
                    Event::Leaf => {
                        ...
                        element.assign(
                            "x",
                            index.x - 5.0 + options.time_lines.len() as f32 * width_timeline,
                        );
                        ...
                        match options.gene_internal {
                            true => {
                                ...
                                if options.node_colors.len() > 0 {
                                    ...
                                    element.assign(
                                        "class",
                                        "gene_".to_owned() + &idx_rcgen.to_string(),
                                    );
                                }
                                let txt = Text::new(&index.name);
                                element.append(txt);
    ...
    if options.thickness_disp_score {
        ...
        let added_style = " .rouge { font-size: ".to_owned()
            + &config.gene_police_size.to_string()
            + "px; fill:red ; } ";
        // Je passe en str pour l'ajouter
        let add_style_str: &str = &added_style;
    ...
    for index in &sp_tree.arena {
        ...
        match sp_tree.is_leaf(index.idx) {
            true => {
                let txt_size =
                    index.name.len() as f32 * config.species_police_size.parse::<f32>().unwrap();
    ...
    while i_trans < unique_transfers.len() {
        ...
        info!(
            "[draw_sptree_gntrees] Additional transfer {}: {}=>{} Score = {}",
            i_trans, start, end, score
        );
        i_trans = i_trans + 1;
        if score > options.thickness_thresh {
            info!(
                "[draw_sptree_gntrees] Selecting additional transfer {:?} {:?}",
                start, end
            );
            ...
            info!(
                "[draw_sptree_gntrees] Adding additional transfer {:?} {:?} with opacity {}",
                start, end, opacity
            );
        ...
        for (node_name, node_color) in time_line.iter() {
            ...
            match node {
                ...
                Ok(node) => {
                    ...
                    time_line_nodes.push(TimeLineNode {
...
pub fn get_chemin_transfer(
    ...
    // ligne horizontale
    let mut data = "M".to_owned()
        + &x1.to_string()
        + " "
        + &initial_y1.to_string()
        + " L "
        + &x1.to_string()
        + " "
        + &y1.to_string();
    ...
    if stroke != 3 {
        data = data.to_owned()
            ...
            + &x1.to_string()
            + " "
            + &(initial_y1 - PIPEBLOCK / 4.0).to_string()
            + "L "
