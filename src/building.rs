...
use crate::arena::{
    bilan_mappings, bilan_mappings_reti, center_gene_nodes, check_vertical_contour_postorder,
    map_species_trees, move_dupli_mappings, move_species_mappings, process_fl, set_species_width,
    species_uniformisation, uniformise_gene_leaves_y_values,
};
use crate::arena::{
    check_contour_postorder, check_contour_postorder_tidy_tree, cladogramme, fusion_mod_xy,
    knuth_layout, real_length, set_leaves_y_values, set_middle_postorder, shift_mod_xy,
    shift_nodes_y_values,
};
use crate::arena::{
    check_for_obsolete, find_rgtrees, find_sptrees, make_invisible, scale_heigth, scale_width,
};
use crate::arena::{get_descendant, newick2tree, xml2tree};
use crate::drawing::{draw_sptree_gntrees, draw_tree};
use crate::thirdlevel::{
    check_optimisation, classify_transfer, get_gtransfer, optimisation, reorder_transfers,
};
...
pub fn add_virtual_roots(
    ...
    if global_roots.len() == 1 {
        let nom_droite =
            "VIRTUAL_".to_string().to_owned() + &level.to_string() + "_" + &(*pere - 1).to_string();
        let nom_pere =
            "ROOT_".to_string().to_owned() + &level.to_string() + "_" + &(*pere).to_string();
        ...
        if *pere > 0 {
            let mut my_vect: Vec<String> = (0..(*pere + 1))
                .map(|n| "ROOT_".to_string().to_owned() + &level.to_string() + "_" + &n.to_string())
                .collect();
            *level = *level + 1;
            *pere = 0;
...
pub fn read_recphyloxml_multi(
    ...
    for spnode in spnodes {
        ...
        let spnode = doc
            .get_node(spnode)
            .expect("Unable to get the Node associated to this nodeId");
...
pub fn phyloxml_processing(
    ...
    for collapsed_node in &options.collapsed_nodes {
        let sw = match tree.get_index(collapsed_node.to_string()) {
            ...
            Err(_err) => {
                eprintln!(
                    "[phyloxml_processing] ERROR Unable to find node {:?}",
                    collapsed_node.to_string()
                );
                std::process::exit(1);
            }
        };
    ...
    if options.height != 1.0 {
        scale_heigth(&mut tree, options.height)
    };
    // ---------------------------------------------------------
    // OPTIONAL Scale the width if needed
    // ---------------------------------------------------------
    if options.width != 1.0 {
        scale_width(&mut tree, options.width)
    };
    // ---------------------------------------------------------
    // Option : real_length
    // ---------------------------------------------------------
...
pub fn recphyloxml_processing(
    ...
    for switch in &options.switches {
        let sw = match sp_tree.get_index(switch.to_string()) {
            ...
            Err(_err) => {
                eprintln!(
                    "[recphyloxml_processing] ERROR Unable to find node {:?}",
                    switch.to_string()
                );
                std::process::exit(1);
            }
    ...
    if options.real_length_flag {
        ...
        if min_dist == 0.0 {
            ...
            eprintln!(
                "It is not possible create an 'upper' tree presenting branches of zero length."
            );
            std::process::exit(1);
        }
    ...
    for (hybrid_name, hybrid) in &hybrids {
        ...
        for (index_node, node) in nodes {
            ...
            {
                gene_trees[*index_node].arena[*node].x =
                    gene_trees[*index_node].arena[children[0]].x;
            }
        }
    ...
    for (name1, name2) in &options.hybrid {
        ...
        println!(
            "Merging nodes {} [{}] and {}  [{}]",
            h1, sp_tree.arena[h1].name, h2, sp_tree.arena[h2].name
        );
    ...
    // ---------------------------------------------------------
    if options.height != 1.0 {
        scale_heigth(&mut sp_tree, options.height)
    };
    // ---------------------------------------------------------
    // Option Scale the width if needed
    // ---------------------------------------------------------
    if options.width != 1.0 {
        scale_width(&mut sp_tree, options.width)
    };
    // ---------------------------------------------------------
    ...
    for (hybrid_name, hybrid) in &hybrids {
        ...
        println!(
            "Hybrid {}: {:?} between {} and {}",
            hybrid_name, hybrid, sp_tree.arena[hybrid1.0].name, sp_tree.arena[hybrid2.0].name
        );
    ...
    for (_hybrid_name, hybrid) in &hybrids {
        ...
        for root in vec![root1, root2].iter() {
            ...
            for (index_node, node) in nodes {
                let children = &gene_trees[*index_node].arena[*node].children;
                // gere le cas ou la racine est a gauche du fils gauche ( ou a droite du fils droit)
                if gene_trees[*index_node].arena[*node].x
                    < gene_trees[*index_node].arena[children[1]].x
                {
                    gene_trees[*index_node].arena[*node].x =
                        gene_trees[*index_node].arena[children[1]].x;
                }
